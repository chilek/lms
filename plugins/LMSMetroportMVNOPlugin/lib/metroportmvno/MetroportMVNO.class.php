...
//        $this->dbtype = ConfigHelper::getConfig('database.type');
     ...
     * @param string $mmcsUserId MMSC user ID
     * @return int
     * @throws
     */
    public function setCustomerExtid(int $lmsCustomerId, string $mmcsUserId)
    {
    ...
    public function setCustomerExtidMessage(array $params)
    {
        ...
        if ($params['matching_result']) {
            $msg = 'LMS customer ' . $params['lms_customer_lsatname'] . $params['lms_customer_name'] . '(#' . $params['lms_customer_id'] . ')' . trans('has been bound with MMSC user') . $params['mmsc_user_code_name'];
        } /*else {
