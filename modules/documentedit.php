...
if(isset($_GET['action']) && $_GET['action'] == 'confirm')
{
	if(!empty($_POST['marks']))
	{
			...
			$DB->Execute('UPDATE documents SET sdate=?NOW?, closed=1 WHERE id=?
...
$document = $DB->GetRow('SELECT documents.id AS id, closed, type, number, template,
	cdate, sdate, numberplanid, title, fromdate, todate, description, divisionid, documents.customerid
	FROM documents
...
if(isset($_POST['document']))
{
	...
	if (!$error) {
		...
		$DB->Execute('UPDATE documents SET type=?, closed=?, sdate=?, number=?, numberplanid=?, fullnumber=?
				WHERE id=?',
