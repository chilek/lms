...
switch($type)
{
	...
	case 'invoices': /********************************************/
		...
		header('Location: ?m=invoice&fetchallinvoices=1' . (isset($_GET['jpk']) ? '&jpk=' . $_GET['jpk'] : '')
			...
			.(!empty($_POST['group']) ? '&groupid[]=' . implode('&groupid[]=', array_filter($_POST['group'], 'intval')) : '')
			.(!empty($_POST['customer_type']) ? '&customertype='.intval($_POST['customer_type']) : '')
			.(!empty($_POST['numberplan']) ? '&numberplanid[]=' . implode('&numberplanid[]=', array_filter($_POST['numberplan'], 'intval')) : '')
