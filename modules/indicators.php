	...
	case 'critical':
		if (ConfigHelper::CheckPrivilege('helpdesk_administration') || ConfigHelper::CheckPrivilege('helpdesk_operation')) {
			...
			if ($count == 1) {
				...
				$redirect = '?m=rtqueueview&catid=all&state=-1&priority=' . RT_PRIORITY_CRITICAL . '&ownerid=-2&s=-2&unread=-1&o=lastmodified';
		}
		break;
	case 'urgent':
		if (ConfigHelper::CheckPrivilege('helpdesk_administration') || ConfigHelper::CheckPrivilege('helpdesk_operation')) {
			...
			if ($count == 1) {
				...
				$redirect = '?m=rtqueueview&catid=all&state=-1&priority=' . RT_PRIORITY_URGENT . '&ownerid=-1&s=-2&unread=-1&o=lastmodified';
		}
		break;
	case 'unread':
		if (ConfigHelper::CheckPrivilege('helpdesk_administration') || ConfigHelper::CheckPrivilege('helpdesk_operation')) {
			...
			if ($count == 1) {
				...
				$redirect = '?m=rtqueueview&catid=all&state=0&priority=all&ownerid=-1&s=-2&unread=1&o=lastmodified';
		}
		break;
}
