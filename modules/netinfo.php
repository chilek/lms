...
if (isset($_GET['action'], $_GET['id']) && $_GET['action'] === 'halve' && ConfigHelper::checkPrivilege('network_management')) {
    try {
        $LMS->HalveNetwork(intval($_GET['id']));
        $SESSION->redirect('?m=netinfo&id=' . intval($_GET['id']));
    } catch (Throwable $e) {
        $error['halve'] = trans($e->getMessage());
    }
}
