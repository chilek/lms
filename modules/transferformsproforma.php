...
function main_fill($x,$y,$scale)
{
    ...
    $pdf->addtext(15*$scale+$x,250*$scale+$y,30*$scale, iconv('UTF-8', 'ISO-8859-2',trans('Payment for pro forma invoice No. $a', $invoice['t_number'])));
}
...
function simple_fill_mip($x,$y,$scale)
{
    ...
    $font_size=30;
    while ($pdf->getTextWidth($font_size*$scale,trans('Pro Forma Invoice No. $a', $invoice['t_number']))>135)
	$font_size=$font_size-1;    
    $pdf->addtext(15*$scale+$x,385*$scale+$y,$font_size*$scale,trans('Pro Forma Invoice No. $a', $invoice['t_number']));
...
$ids = $DB->GetCol('SELECT id FROM documents d
        WHERE cdate >= ? AND cdate <= ? AND type = 6 AND closed = 0 '
