...
switch ($_GET['action']) {
    case 'open':
        if (($event.closed && $aee && ((time() - "$event.closeddate") < $aee)) || ConfigHelper::checkPrivilege('superuser')) {
    ...
    case 'close':
        ...
        break;
    case 'assign':
        if (!$event.closed || ($event.closed && $aee && ((time() - "$event.closeddate") < $aee)) || ConfigHelper::checkPrivilege('superuser')) {
