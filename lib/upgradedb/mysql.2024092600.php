...
$vcustomerassignments_exists = $this->ResourceExists('vcustomerassignments', LMSDB::RESOURCE_TYPE_VIEW);
if ($vcustomerassignments_exists) {
...
$this->Execute(
    "CREATE FUNCTION UNIX_TIMESTAMP64() RETURNS BIGINT DETERMINISTIC
        RETURN TIMESTAMPDIFF(SECOND, DATE '1970-01-01', UTC_TIMESTAMP())"
);
...
if ($this->ResourceExists('customerassignments_insert_trigger', LMSDB::RESOURCE_TYPE_TRIGGER)) {
    $this->Execute("DROP TRIGGER customerassignments_insert_trigger");
    $this->Execute(
...
$this->Execute("UPDATE dbinfo SET keyvalue = ? WHERE keytype = ?", array('2024092600', 'dbversion'));
