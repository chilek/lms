...
$this->Execute("ALTER TABLE liabilities ALTER COLUMN value TYPE numeric(9,3)");
$this->Execute("ALTER TABLE liabilities ALTER COLUMN netvalue TYPE numeric(9,3)");
$this->Execute("ALTER TABLE assignments ALTER COLUMN vdiscount TYPE numeric(9,3)");
$this->Execute("ALTER TABLE invoicecontents ALTER COLUMN vdiscount TYPE numeric(9,3)");
...
$this->Execute(
    ...
    SELECT
        ...
        CASE
            WHEN (d.flags & ?) > 0 THEN round(ic.value, 3) - round(ic2.value, 3)
            ELSE round(ic.value / (1 + t.value / 100), 3) - round(ic2.value / (1 + t.value / 100), 3)
            END AS diff_netprice,
        CASE
            WHEN (d.flags & ?) > 0 THEN round(ic.value * (1 + t.value / 100), 3) - round(ic2.value * (1 + t.value / 100), 3)
