...
$this->Execute("ALTER TABLE ewx_stm_channels MODIFY cid int(11) NULL");
$this->Execute("ALTER TABLE ewx_stm_channels ALTER COLUMN cid SET DEFAULT NULL");

$this->Execute("UPDATE ewx_stm_channels SET cid = NULL WHERE cid = 0");
$ids = $this->GetCol("SELECT id FROM ewx_channels");
if (empty($ids))
	$this->Execute("UPDATE ewx_stm_channels SET cid = NULL AND cid IS NOT NULL");
else {
	$sql_ids = implode(',', $ids);
