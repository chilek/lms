...
class LMS
{
	...
	function GetUserList($order='username,asc', $state=NULL, $network=NULL, $usergroup=NULL)
	{
		...
		if($userlist = $this->DB->GetAll( 
				'SELECT users.id AS id, '.$this->DB->Concat('UPPER(lastname)',"' '",'users.name').' AS username, status, email, phone1, users.address, gguin, nip, pesel, zip, city, info, message, '
				...
				.' GROUP BY users.id, lastname, users.name, status, email, phone1, users.address, gguin, nip, pesel, zip, city, info, message '
...
//		$ticket['requestor'] = str_replace(' <'.$ticket['requestoremail'].'>','',$ticket['requestor']);
	...
	function RTSearch($search, $order='createtime,desc')
	{
		...
		if($search['name'])
			$where .= 'AND (requestor ?LIKE? \'%'.$search['name'].'%\' OR '.$this->DB->Concat('users.lastname',"' '",'users.name').' ?LIKE? \'%'.$search['name'].'%\') ';
