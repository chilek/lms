...
class LMSHelpdeskManager extends LMSManager implements LMSHelpdeskManagerInterface
{
	...
	$del = 0;
        if ($result = $this->db->GetAll('SELECT q.id, name, email, description, newticketsubject, newticketbody,
				...
				FROM rtqueues q'
				. (!ConfigHelper::checkPrivilege('superuser') && !ConfigHelper::checkPrivilege('helpdesk_extend_operation') ? ' JOIN rtrights r ON r.queueid = q.id
    ...
    public function GetTicketContents($id)
    {
        ...
        $ticket = $this->db->GetRow('SELECT t.id AS ticketid, t.queueid, rtqueues.name AS queuename,
				...
				WHERE 1=1 '
				. (!ConfigHelper::checkPrivilege('superuser') && !ConfigHelper::checkPrivilege('helpdesk_extend_operation') ? ' AND t.deleted = 0' : '')
				. ('AND t.id = ?'), array($id));
        ...
        $ticket['messages'] = $this->db->GetAll(
				...
				WHERE 1=1 '
				. (!ConfigHelper::checkPrivilege('superuser') && !ConfigHelper::checkPrivilege('helpdesk_extend_operation') ? 'AND rtmessages.deleted = 0' : '')
				. ('AND ticketid = ?)')
