...
class LMSEventManager extends LMSManager implements LMSEventManagerInterface
{
    public function EventAdd($event)
    {
        $args = array(
            ...
            SYSLOG::RES_CUST => empty($event['custid']) ?: $event['custid'],
            'type' => $event['type'],
            SYSLOG::RES_ADDRESS => empty($event['address_id']) ?: $event['address_id'],
            SYSLOG::RES_NODE => $event['nodeid'],
            SYSLOG::RES_TICKET => empty($event['ticketid']) ?: $event['ticketid'],
        );
    ...
    public function EventUpdate($event)
    {
        $args = array(
            ...
            'type' => $event['type'],
            SYSLOG::RES_ADDRESS => empty($event['address_id']) ?: $event['address_id'],
            SYSLOG::RES_NODE => empty($event['nodeid']) ?: $event['nodeid'],
            SYSLOG::RES_TICKET => empty($event['helpdesk']) ?: $event['helpdesk'],
            'moddate' => time(),
