...
class LMSCashManager extends LMSManager implements LMSCashManagerInterface
{
	...
	public function CashImportParseFile($filename, $contents, $patterns, $quiet = true) {
		...
		foreach ($file as $line) {
			...
			if (!$id && !empty($pattern['invoice_regexp'])) {
				if (preg_match($pattern['invoice_regexp'], $theline, $matches)) {
					...
					if ($invid && $invyear) {
						...
						$id = $this->db->GetOne('SELECT customerid FROM documents
			...
			if (!$id)
				if (!empty($dstaccount))
					$id = $this->db->GetOne('SELECT customerid FROM customercontacts
			...
			if (!$id && $name && $lastname) {
				$uids = $this->db->GetCol('SELECT id FROM customers WHERE UPPER(lastname)=UPPER(?) and UPPER(name)=UPPER(?)',
					array($lastname, $name));
				if (count($uids) == 1)
					$id = $uids[0];
