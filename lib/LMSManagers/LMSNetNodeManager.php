...
class LMSNetNodeManager extends LMSManager implements LMSNetNodeManagerInterface
{
    ...
    public function NetNodeAdd($netnodedata)
    {
        ...
        $args = array(
            ...
            'longitude'       => empty($netnodedata['longitude']) ? null : str_replace(',', '.', $netnodedata['longitude']),
            'latitude'        => empty($netnodedata['latitude'])  ? null  : str_replace(',', '.', $netnodedata['latitude']),
            'ownership'       => $netnodedata['ownership'],
            'coowner'         => $netnodedata['coowner'],
            'uip'             => $netnodedata['uip'],
            'miar'            => $netnodedata['miar'],
            'createtime'      => time(),
            'divisionid'      => empty($netnodedata['divisionid']) ? null : $netnodedata['divisionid'],
