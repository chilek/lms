...
class LMSNetworkManager extends LMSManager implements LMSNetworkManagerInterface
{
    ...
    public function HalveNetwork(int $netid): int
    {
        ...
        $usable_range = static function (int $net, int $broadcast, int $prefix) use ($all_assignable): array {
            if ($all_assignable || $prefix >= 31) {
                return [$net, $broadcast];
            }
            return [$net + 1, $broadcast - 1];
        };

        [$old_usable_start, $old_usable_end] = $usable_range($first_network, $old_broadcast, $old_prefix);
        [$first_usable_start, $first_usable_end] = $usable_range($first_network, $first_broadcast, $prefix);
        [$second_usable_start, $second_usable_end] = $usable_range($second_network, $second_broadcast, $prefix);

        $ip_long = static function ($value): int {
            if (is_string($value) && strpos($value, '.') !== false) {
                return intval(ip_long($value));
            }
            return intval($value);
        };

        $in_range = static function (int $ip, int $start, int $end): bool {
        ...
        if (!empty($network['gateway'])) {
            ...
            } elseif ($in_range($gateway, $first_usable_start, $first_usable_end)) {
                $gateways['first'] = $network['gateway'];
            } elseif ($in_range($gateway, $second_usable_start, $second_usable_end)) {
