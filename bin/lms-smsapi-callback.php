...
$status = strtolower($_GET['status_name']);

if (!preg_match('/^(sent|delivered|undelivered|rejected|queue)$/', $status)) {
    header('Content-Type: text/plain');
    die('OK');
}

if (!isset($_GET['donedate'])) {
    header('Content-Type: text/plain');
    die('OK');
}
...
switch ($status) {
    ...
    case 'rejected':
        $DB->Execute(
            'UPDATE messageitems SET status = ?, lastdate = ? WHERE id = ?',
            array(MSG_ERROR, $date, $msgitemid)
        );
        break;
    case 'queue':
        $DB->Execute(
