...
if (empty($types) || in_array('contracts', $types)) {
	...
	$customers = $DB->GetAll("SELECT c.id, c.pin, c.lastname, c.name,
		...
		array(CONTACT_EMAIL, CONTACT_EMAIL, CONTACT_DISABLED, CONTACT_DISABLED, CONTACT_EMAIL, $days, $days));
...
if (empty($types) || in_array('debtors', $types)) {
	...
	$customers = $DB->GetAll("SELECT c.id, c.pin, c.lastname, c.name,
		...
		HAVING SUM(value) < ?", array(CONTACT_EMAIL, CONTACT_EMAIL, CONTACT_DISABLED, CONTACT_DISABLED, CONTACT_EMAIL, $days, DOC_RECEIPT, DOC_CNOTE, DOC_INVOICE, DOC_DNOTE, $days, $limit));
...
if (empty($types) || in_array('reminder', $types)) {
	...
	$documents = $DB->GetAll("SELECT d.id AS docid, c.id, c.pin, d.name,
		...
		array(CONTACT_INVOICES, CONTACT_INVOICES, CONTACT_DISABLED, CONTACT_DISABLED, CONTACT_MOBILE, CONTACT_MOBILE,
...
if (empty($types) || in_array('notes', $types)) {
	$documents = $DB->GetAll("SELECT d.id AS docid, c.id, c.pin, d.name,
		...
		WHERE (c.invoicenotice IS NULL OR c.invoicenotice = 0) AND d.type = ?
			AND d.cdate >= ? AND d.cdate <= ?",
		array(CONTACT_EMAIL, CONTACT_EMAIL, CONTACT_DISABLED, CONTACT_DISABLED, CONTACT_MOBILE, CONTACT_MOBILE,
...
if (empty($types) || in_array('warnings', $types)) {
	$customers = $DB->GetAll("SELECT c.id, (" . $DB->Concat('c.lastname', "' '", 'c.name') . ") AS name,
		...
		array(CONTACT_EMAIL, CONTACT_EMAIL, CONTACT_DISABLED, CONTACT_DISABLED, 
