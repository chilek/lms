...
function get_city_ids()
{
    ...
    $cities = $DB->GetAll(
        'SELECT LOWER(ls.name) AS state_name, LOWER(ld.name) AS district_name,
...
function convert_pna_to_teryt($data)
{
    ...
    if (mb_strlen($city_name) != mb_strlen($data[CITY]) && isset($cities_with_sections[$city_name])) {
        $borough_name = $city_name = preg_replace(
            '/^.+[[:blank:]]+\((.+)\)$/',
            ($city_name == 'warszawa' ? 'warszawa-' : '') . '$1',
    ...
    foreach ($house_numbers as $house) {
        $DB->Execute(
            'INSERT INTO pna (zip, cityid, cityname, streetid, streetname, fromnumber, fromletter, tonumber, toletter, parity)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
