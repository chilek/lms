...
if (defined('USERPANEL_SETUPMODE')) {
            ...
            foreach ($_POST['userchanges'] as $changeid) {
                ...
                if (preg_match('/(phone|email|im)([0-9]+)/', $changes['fieldname'], $matches)) {
                    ...
                    if ($matches[2]) {
                        ...
                        $DB->Execute(
                            'INSERT INTO customercontacts (contact, customerid, type) VALUES(?, ?, ?)',
                    ...
                    switch ($changes['fieldname']) {
                        ...
                        case 'ten':
                            ...
                            $DB->Execute(
                                'UPDATE customers SET '.$changes['fieldname'].' = ? WHERE id = ?',
                        ...
                        case 'city':
                            ...
                            if (!isset($addresses[$changes['customerid']])) {
                                $addresses[$changes['customerid']] = $DB->GetOne(
                                    'SELECT address_id FROM customer_addresses WHERE customer_id = ? AND type = ?',
    ...
    function module_submit_changes_delete()
    {
        ...
        if (isset($_POST['userchanges'])) {
            ...
            foreach ($_POST['userchanges'] as $changeid) {
                $change = $DB->GetRow(
                    'SELECT customerid, fieldname FROM up_info_changes WHERE id = ?',
                    array($changeid)
                );
