...
function invoice_title($x,$y) 
{
    ...
    $font_size = 16;
    $y=$y-text_align_left($x,$y,$font_size,iconv("UTF-8","ISO-8859-2//TRANSLIT",$invoice['type']));

    if($invoice['type']==trans('DUPLICATE'))
    {
	    $font_size = 12;
...
function invoice_data($x,$y,$width,$font_size,$margin) 
{
    ...
    // Kolumna druga będzie miała rozmiar ustalany dynamicznie
...
function invoice_body() 
{
	...
	default:
	    if(file_exists($template))
		    ...
		    require(SYS_DIR.'/modules/'.$template);
    }
}
